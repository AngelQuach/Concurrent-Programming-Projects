 ...
 */
int process_data(CircularQueue *q, URL_Array *PNG_url, CURL *curl_handle, RECV_BUF *p_recv_buf)
{
    CURLcode res;
    long response_code;
    ...
    if ( strstr(ct, CT_HTML) ) {
        return process_html(q, curl_handle, p_recv_buf);
    } else if ( strstr(ct, CT_PNG) ) {
        return process_png(curl_handle, PNG_url, p_recv_buf);
    }
    return 0;
}
...
int process_png(CURL *curl_handle, URL_Array *PNG_url, RECV_BUF *p_recv_buf)
{
    ...
    if (eurl != NULL) {
        ...
        if(buf[1] == 0x50 && buf[2] == 0x4E && buf[3] == 0x47){        /* Check if file is a png */
            /* Add to png list */
            while(addPNGURL(PNG_url, eurl) != 0);                      /* Busy adding url to the list */
            //printf("Successfully added %s, %d\n", PNG_url->urls[PNG_url->count -1], (PNG_url->count) - 1);
